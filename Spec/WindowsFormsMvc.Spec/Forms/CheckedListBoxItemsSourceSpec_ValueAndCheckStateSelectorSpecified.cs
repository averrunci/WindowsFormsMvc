...
[Context("The value and the check state value selector are specified")]
class CheckedListBoxItemsSourceSpec_ValueAndCheckStateSelectorSpecified : ItemsSourceSpecContext<CheckedListBox, TestItem>
{
    CheckedListBox CheckedListBox { get; } = new();

    protected override ItemsSource<CheckedListBox, TestItem> CreateItemsSource()
        => new CheckedListBoxItemsSource<TestItem>(CheckedListBox, item => item.ValueProperty, item => item.CheckStateProperty);
    ...
    void Ex11()
    {
        ...
        {
            Items[2].CheckState = CheckState.Unchecked;
            Items[3].CheckState = CheckState.Indeterminate;
            Items[4].CheckState = CheckState.Checked;
        });
        Then("the check state of the checked list box should be changed", () =>
