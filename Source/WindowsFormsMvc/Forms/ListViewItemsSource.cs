...
public class ListViewItemsSource<TItem> : ItemsSource<ListView, TItem>
{
    ...
    private readonly List<ListViewItemContext<TItem>> listViewItemContexts = new();
    ...
    protected override void InsertItem(int index, TItem item)
    {
        ...
        if (CheckedSelector is not null) listViewItemContext.BindChecked(CheckedSelector);
    }
    ...
    {
        if (CheckedSelector is not null) listViewItem.Checked = CheckedSelector(item).Value;

        if (CheckedMember is not null)
        {
            var checkedValue = item?.GetType().GetProperty(CheckedMember)?.GetValue(item);
            if (checkedValue is not bool @checked) return;
    ...
    {
        if (CheckedSelector is not null) CheckedSelector(GetItems()[e.Item.Index]).Value = e.Item.Checked;
